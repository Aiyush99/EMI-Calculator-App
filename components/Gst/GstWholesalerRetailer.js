...
export default function GstWholeSalerRetailer() {
    ...
    const handleCostOfGoods = (CostOfGoods)=>{
        setCostOfGoods(CostOfGoods);
    }
    ...
    const ProductionGoods = Number(CostOfGoods);
    const ProfitRatio = Number(profitRatio);
    const TotalProductionGoods = Number(ProductionGoods * ProfitRatio) / 100 + ProductionGoods;
    const TotalTax = (TotalProductionGoods * gst) / 100
    const CGST = Number(TotalTax) / 2;
    const IGST = Number(TotalTax) / 2;

   
    const item = [
        { name: 'Total Production Cost', code: '#7ea',value:TotalProductionGoods.toFixed(0)},
    ...
    return (
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <Fragment>
            ...
            <TextInput autoFocus style={{width:"95%",marginLeft:10,marginTop:30}} keyboardType="number-pad" maxLength={9} label="Cost Of Goods"value={CostOfGoods}onChangeText={handleCostOfGoods}/>
